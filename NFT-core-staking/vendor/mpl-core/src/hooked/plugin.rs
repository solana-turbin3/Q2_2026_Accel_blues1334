...
// `#[inline(never)]`: keep this large frame out of its callers (`Asset::deserialize`,
// `Collection::deserialize`), which were only barely over the 4096-byte SBF stack limit
// because they inlined this builder. The accumulator below is boxed so the ~19-field
// `PluginsList` lives on the heap instead of this function's stack frame.
#[inline(never)]
pub(crate) fn registry_records_to_plugin_list(
    ...
    let result = registry_records
        .iter()
        .try_fold(Box::new(PluginsList::default()), |mut acc, record| {
    ...
    // Unbox into the return slot (sret) so the full `PluginsList` is materialized in the
    // caller, not on this frame.
