...
mod tests {
    ...
    fn test_refund() {
        ...
        assert!(svm.get_account(&escrow).map(|a| a.lamports == 0 && a.data.is_empty()).unwrap_or(true),
                "escrow account should be closed");
        assert!(svm.get_account(&vault).map(|a| a.lamports == 0 && a.data.is_empty()).unwrap_or(true),
                "vault should be closed");

        let maker_balance = read_token_balance(&svm, &maker_ata_a);
        assert_eq!(maker_balance, 1_000_000_000, "maker should have full balance restored");

        // Sanity: amount_to_give is the value originally sent and now returned
    ...
    fn test_take() {
        ...
        let taker_a_bal = read_token_balance(&svm, &taker_ata_a);
        assert_eq!(taker_a_bal, amount_to_give, "taker should receive amount_to_give of mint_a");

        let maker_b_bal = read_token_balance(&svm, &maker_ata_b);
        assert_eq!(maker_b_bal, amount_to_receive, "maker should receive amount_to_receive of mint_b");

        let taker_b_bal = read_token_balance(&svm, &taker_ata_b);
        assert_eq!(taker_b_bal, amount_to_receive, "taker mint_b should be debited");

        // maker_ata_a is untouched after make (vault held the tokens)
        let maker_a_bal = read_token_balance(&svm, &maker_ata_a);
        assert_eq!(maker_a_bal, 1_000_000_000 - amount_to_give);
